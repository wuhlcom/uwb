...
public class TcpServer {
    ...
    public void run(int port) {
		...
		*/
        EventLoopGroup bossGroup = new NioEventLoopGroup(conf.getBossThreadCount()); // (1)
        EventLoopGroup workerGroup = new NioEventLoopGroup(conf.getWorkerThreadCount());
        try {
            ServerBootstrap b = new ServerBootstrap(); // (2)
            b.group(bossGroup, workerGroup).channel(NioServerSocketChannel.class) // (3)
                    .childHandler(new TcpHanlderInitializer())
                    .option(ChannelOption.SO_BROADCAST, conf.isSoBroadcast()) // (4)
                    .option(ChannelOption.SO_REUSEADDR, conf.isSoReuseaddr())
                    .option(ChannelOption.SO_TIMEOUT, conf.getSoTimeout())
                    .option(ChannelOption.SO_RCVBUF, conf.getSoRcvbuf())
                    .option(ChannelOption.TCP_NODELAY, conf.isNodelay())
                    .option(ChannelOption.SO_SNDBUF, conf.getSoSndbuf())
                    .option(ChannelOption.SO_TIMEOUT, conf.getSoTimeout())
                    // 服务端接受连接的队列长度，如果队列已满，客户端连接将被拒绝
                    .option(ChannelOption.SO_BACKLOG, conf.getSoBacklog()) // (5)
                    .childOption(ChannelOption.SO_KEEPALIVE, conf.isSoKeepAlive()); // (6)

            // 绑定端口，开始接收进来的连接
            ChannelFuture f = b.bind(port).sync(); // (7)
            System.out.println("Tcp server start listen at " + port);
            // 等待服务器 socket 关闭 。
