...
import com.zhilutec.cloud.common.validators.RouteValidator;

import com.zhilutec.cloud.pojo.CommonRoute;
import com.zhilutec.cloud.pojo.FilterInfo;
import com.zhilutec.cloud.pojo.PathRoute;
import com.zhilutec.cloud.pojo.RouteInfo;
import com.zhilutec.cloud.service.IRedisService;
import com.zhilutec.cloud.service.IRouteService;
    ...
    private List<FilterDefinition> filterList(CommonRoute customerRoute) {
        ...
        if (filterInfos != null && filterInfos.size() > 0) {
            for (FilterInfo filterInfo : filterInfos) {
                ...
                if (filterType.equals(ConstantUtil.ADD_REQUEST_HEADER_FILTER) ||
                        filterType.equals(ConstantUtil.ADD_REQUEST_PARAMETER_FILTER) ||
                        filterType.equals(ConstantUtil.ADD_RESPONSE_HEADER_FILTER)) {
                    // filters:
                    // - AddRequestHeader=X-Request-Foo, Bar
                    String values[] = filter.split(",");
                    filterParams.put(GatewayFilter.NAME_KEY, values[0]);
                    filterParams.put(GatewayFilter.VALUE_KEY, values[1]);
                } else if (filterType.equals(ConstantUtil.HYSTRIX_FILTER)) {
                    // {"name":"xxx","fallbackUri":"xxx","setter":""}
                    // {"name":"xxx"}
                    filterParams = JSON.parseObject(filter, HashMap.class);
                } else if (filterType.equals(ConstantUtil.PREFIX_PATH_FILTER)) {
                    // filters:
                    // - PrefixPath=/mypath
                    filterParams.put(PrefixPathGatewayFilterFactory.PREFIX_KEY, filter);
                } else if (filterType.equals(ConstantUtil.PRESERVE_HOST_HEADER_FILTER)) {
                    // filters:
                    // - PreserveHostHeader
                    // 此过滤不需要参数
                } else if (filterType.equals(ConstantUtil.REUQEST_RATE_LIMITER_FILTER)) {
                    // 每秒最大访问次数
                    // filterParams.put("redis-rate-limiter.replenishRate", "2");
                    // 令牌桶最大容量
                    // filterParams.put("redis-rate-limiter.burstCapacity", "3");
                    ...
                    // filterParams.put(PrefixPathGatewayFilterFactory.PREFIX_KEY, filter);
                    filterParams = JSON.parseObject(filter, HashMap.class);
                } else if (filterType.equals(ConstantUtil.REDIRECT_TO_FILTER)) {
                    filterParams.put(PrefixPathGatewayFilterFactory.PREFIX_KEY, filter);
                } else if (filterType.equals(ConstantUtil.REMOVE_REQUEST_HEADER_FILTER) ||
                        filterType.equals(ConstantUtil.REMOVE_RESPONSE_HEADER_FILTER)) {
                    // filters:
                    // - RemoveResponseHeader=X-Response-Foo
                    filterParams.put(GatewayFilter.NAME_KEY, filter);
                } else if (filterType.equals(ConstantUtil.REWRITE_PATH_FILTER)) {
                    // filters:
                ...
                } else if (filterType.equals(ConstantUtil.SAVE_SESSION_FILTER)) {
                    // filters:
                    // - RewritePath=/foo/(?<segment>.*), /$\{segment}
                    // filters:
                    // - SaveSession
                    // 不需要传参数
                } else if (filterType.equals(ConstantUtil.SET_PATH_FILTER)) {
                    // predicates:
                    // - Path=/foo/{segment}
                    // filters:
                    // - SetPath=/{segment}
                    filterParams.put(SetPathGatewayFilterFactory.TEMPLATE_KEY, filter);
                } else if (filterType.equals(ConstantUtil.SET_RESPONSE_HEADER_FILTER)) {
                    // filters:
                    // - SetResponseHeader=X-Response-Foo, Bar
                    String values[] = filter.split(",");
                    filterParams.put(GatewayFilter.NAME_KEY, values[0]);
                    filterParams.put(GatewayFilter.VALUE_KEY, values[1]);
                } else if (filterType.equals(ConstantUtil.SET_STATUS_FILTER)) {
                    // filters:
                    // - SetStatus=401
                    filterParams.put(SetStatusGatewayFilterFactory.STATUS_KEY, filter);
                } else if (filterType.equals(ConstantUtil.STRIP_PREFIX_FILTER)) {
                    // routes:
                    // - id: nameRoot
                ...
                } else if (filterType.equals(ConstantUtil.RETRY_FILTER)) {
                    // filters:
                    // - name: Retry
                    // args:
                    // retries: 3
    ...
    private List<PredicateDefinition> predicateList(CommonRoute customerRoute) {
        ...
        if (routeInfos != null && routeInfos.size() > 0) {
            ...
            for (RouteInfo routeInfo : routeInfos) {
                ...
                } else if (routeType.equals(ConstantUtil.COOKIE_ROUTE)) {
                    String values[] = routeInfo.getRoute().split(",");
                ...
                } else if (routeType.equals(ConstantUtil.HEADER_ROUTE)) {
                    String values[] = routeInfo.getRoute().split(",");
                ...
                } else if (routeType.equals(ConstantUtil.QUERY_ROUTE)) {
                    String values[] = routeInfo.getRoute().split(",");
                    if (values.length > 1) {
                ...
                } else if (routeType.equals(ConstantUtil.REMOTE_ROUTE)) {
