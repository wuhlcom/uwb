...
import com.zhilutec.uwb.dao.FileDao;
...
import uwb.common.validator.FileValidator;
...
public class FdfsFileServiceImpl implements IFdfsFileService {
	 ...
	 */
	@Override
	public Integer addDb(String jsonStr) {
	 ...
	 */
	@Override
	public String deleteDb(JSONObject jsonObject) {
		...
		if (fileEntities != null && fileEntities.size() > 0) {
			...
			return Result.ok("文件已删除").toJSONString();
		}
	}
	...
	@Transactional
	@Override
	public String updateDb(JSONObject jsonObject) {
	...
	public String getFilesRs(JSONObject jsonObject) {
		...
		//参数校验
		Result validator = FdfsValidator.getVal(jsonObject);
		...
		//生成地图url
		String uriPort = fdfsWebServer.getWebServerUrl();

		for (FileEntity fileEntity : fileEntities) {
			String downLoadPath = fileEntity.getRemotePath();
			fileEntity.setDownPath("/" + downLoadPath);
			if (uriPort != null && !uriPort.isEmpty()) {
				fileEntity.setPath(uriPort + downLoadPath);
			}
	...
	public String mapSwitch(JSONObject jsonObject) {
		...
		fileEntityParam.setStatus(ConstantUtil.FILE_ON);
		fileEntityParam.setId(id);

		//打开地图，将其它地图设置为不启用
		//查出所有启用状态的地图
		List<FileEntity> fileEntities = this.getFileInfoByStatus(ConstantUtil.FILE_ON);
		FileEntity fileEntityOff = new FileEntity();
		fileEntityOff.setStatus(ConstantUtil.FILE_OFF);
		Example example = new Example(FileEntity.class);
		Example.Criteria criteria = example.createCriteria();
		criteria.andEqualTo("status", ConstantUtil.FILE_ON);
		//将原有的status为1的改为0
		fileDao.updateByExampleSelective(fileEntityOff, example);
