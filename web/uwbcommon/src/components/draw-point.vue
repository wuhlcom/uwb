    ...
    animation (arr) {
      ...
      let darw = () => {
        ...
        arr.forEach(e => {
          if (e.show){
            let lucency = 0
            e.point.forEach((el, index) => {
              if (el) {
                ctx.beginPath()
                if (index === e.point.length - 1) {
                  ctx.arc(el.x * this.proportionWith, el.y * this.proportionHeight, 3.5, 0, Math.PI * 2)
                } else {
                  ctx.arc(el.x * this.proportionWith, el.y * this.proportionHeight, 2, 0, Math.PI * 2)
                }
                if (e.point.length === 1){
                  lucency = 1
                } else {
