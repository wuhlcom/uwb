...
export default {
  ...
  data() {
     var validatePass2 = (rule, value, callback) => {
      if (value === '00:00:00' && this.form.startTime === '00:00:00') {
        return
    ...
    return {
      form: {
        ...
        timeValues: this.data.timeValues,
        forbidden: this.data.forbidden,
        remark: this.data.remark
      },
      strategyUserId: this.data.parentCodes,
      repetition: "",
      level: [
        {
          label: "提示",
  ...
  methods: {
    ...
    upData() {
      this.$http
        .Post(this.$url.strategyModify, this.form)
        .then(res => {})
        .catch(rej => {
  ...
  created() {
    this.getData();
    this.form.strategyUserId = this.data.parentCodes[this.data.parentCodes.length - 1];
  },
  watch: {
    strategyUserId(val) {
